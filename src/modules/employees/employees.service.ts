...
export class EmployeesService {
  ...
  async generateUniqueCombinationId(
    baseId: string,
  ): Promise<string> {
    let uniqueId = baseId;
    ...
    while (
      await this.employeeModel
        .findOne({ employeeId: uniqueId })
        .exec()
    ) {
      const suffix = String(counter);

      uniqueId =
        baseId.substring(0, 8 - suffix.length) + suffix;
  ...
  async generateNextEmployeeId(queryParams?: {
    ...
    const match =
      lastEmployee.employeeId.match(/^EMP(\d+)$/);

    if (!match) {
      const numMatch =
        lastEmployee.employeeId.match(/\d+/);

      const nextNum = numMatch
  ...
  async create(
    ...
    if (
      ...
      createEmployeeDto.confirmPassword &&
      createEmployeeDto.password !==
        createEmployeeDto.confirmPassword
    ) {
    ...
    const personalEmailClean =
      createEmployeeDto.personalEmail
        .toLowerCase()
        .trim();

    const officialEmailClean =
    ...
    if (officialEmailClean) {
      ...
      if (existingByOfficialEmail) {
        throw new ConflictException(
          'Employee with this official email already exists',
        );
      }
    }

    let employeeId =
      createEmployeeDto.employeeId?.trim();

    if (!employeeId) {
      const baseId =
        this.generateCombinationEmployeeId(
      ...
      employeeId =
        await this.generateUniqueCombinationId(
          baseId,
        );
    } else {
      const existingById =
    ...
    try {
      ...
      if (emailToUse) {
        ...
        if (!existingUser) {
          ...
          if (
            ...
            des.includes('supper admin') ||
            des.includes('superadmin')
          ) {
      ...
      console.error(
        'Failed to auto-create user for employee:',
        err,
      );
    }
  ...
  /*
   * ============================================================
   * EMPLOYEES FOR OPPORTUNITY SOURCE DROPDOWN
   * ============================================================
   *
   * This method reads employee records directly from MongoDB.
   *
   * It does NOT contain any manually entered employee names.
   *
   * It returns every employee except employees whose status is
   * explicitly "Inactive".
   *
   * Therefore:
   *
   * Active employee      -> returned
   * Missing status       -> returned
   * Inactive employee    -> not returned
   *
   * Endpoint:
   * GET /employees/source-options
   */
  async findSourceOptions(): Promise<
    {
      id: string;
      employeeId: string;
      firstName: string;
  ...
  > {
    ...
    console.log(
      'EMPLOYEE SOURCE OPTIONS FROM DATABASE:',
    );
  ...
  async findOne(
    ...
    if (!employee) {
      throw new NotFoundException(
        `Employee with identifier ${id} not found`,
      );
    }
  ...
  async update(
    ...
    const oldPersonalEmail =
      employee.personalEmail;

    const oldOfficialEmail =
      employee.officialEmail;

    if (updateEmployeeDto.personalEmail) {
      const emailLower =
        updateEmployeeDto.personalEmail
          .toLowerCase()
          .trim();
    ...
    if (updateEmployeeDto.officialEmail) {
      ...
      if (
        ...
        const existingByOfficial =
          await this.employeeModel
            .findOne({
              officialEmail: officialLower,
            })
            .exec();
    ...
    if (
      ...
      if (existingById) {
        throw new ConflictException(
          `Employee with ID ${updateEmployeeDto.employeeId} already exists`,
        );
      }
    ...
    try {
      ...
      console.error(
        'Failed to sync user details for employee update:',
        err,
      );
    }
  ...
  async remove(
    ...
    try {
      ...
      console.error(
        'Failed to delete user for employee:',
        err,
      );
    }
