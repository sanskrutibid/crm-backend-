...
export class ApiResponseDto<T> {
  @ApiProperty({
    example: true,
    description: 'Indicates if the API request succeeded',
  })
  success: boolean;
...
export class ApiErrorResponseDto {
  ...
  @ApiProperty({
    example: 'Bad Request',
    description: 'HTTP Error category name',
  })
  error: string;

  @ApiProperty({
    example: '2026-05-26T14:04:03.000Z',
    description: 'ISO date timestamp of error occurrence',
  })
  timestamp: string;

  @ApiProperty({
    example: '/api/v1/leads',
    description: 'Request path that raised the exception',
  })
  path: string;
}
